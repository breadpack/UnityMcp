...
public static class PrefabEditTool
{
    [McpServerTool(Name = "unity_prefab_edit"),
     Description(
         "Prefab 편집 스테이지의 진입/저장/명시적 종료/상태를 제어합니다. 이 도구 자체는 편집하지 않습니다 — " +
         "enter 후 generic 씬 도구(unity_set_property/unity_add_component/unity_create_gameobject 등)로 " +
         "편집하고 save_and_exit로 저장·종료합니다. dirty 상태의 exit는 저장 확인창을 띄우지 않고 실패하며, " +
         "변경을 버리려면 discard_and_exit를 명시해야 합니다. enter는 루트 instanceId/path를 반환하고, 편집 모드에서 " +
         "unity_get_hierarchy는 prefab 내부(각 노드 instanceId 포함)를 반환합니다. " +
         "단일 호출 일괄 편집을 원하면 unity_prefab_apply를 쓰세요.")]
    public static async Task<string> Execute(
        UnityConnection connection,
        [Description("수행할 액션: enter, save, exit(clean 상태만), save_and_exit, discard_and_exit, status")] string action,
